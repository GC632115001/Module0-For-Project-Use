...
def get_and_clean_data():
    data = pd.read_csv('assets/software_developer_united_states_1971_20191023_1.csv')
...
query_map[query_map['java'] > 0].apply(lambda s: np.where(s == 1)[0], axis=1).apply(
    lambda s: list(query_map.columns[s]))

# query_map.loc[query_map['python']==1]
